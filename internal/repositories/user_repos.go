...
func (r *RepositoryV1) AddUser(user models.User) (int, error) {
	...
	err := r.db.QueryRow(
		...
		user.Username, user.PasswordHash, user.Name, user.CreatedAt, user.UpdatedAt,
	).Scan(&id)
	if err != nil {
		return 0, err
	}
...
func (r *RepositoryV1) UpdateUser(user models.User) error {
	_, err := r.db.Exec(
		r.ctx,
		`UPDATE users 
		SET username = $2, password_hash = $3, name = $4, updated_at = $5 WHERE id = $1`,
		user.ID, user.Username, user.PasswordHash, user.Name, user.UpdatedAt,
	)
	if err != nil {
		return err
	}
...
func (r *RepositoryV1) GetUserByID(id int) (models.User, error) {
	var user models.User
	err := r.db.QueryRow(r.ctx, `SELECT id, username, name FROM users WHERE id = $1`,
		id).Scan(&user.ID, &user.Username, &user.PasswordHash, &user.Name,
...
func (r *RepositoryV1) GetUserByUsername(username string) (models.User, error) {
	...
	err := r.db.QueryRow(r.ctx, `
			...
			`, username).Scan(
		&user.ID, &user.Username, &user.PasswordHash, &user.Name,
	)
	if err != nil {
		return models.User{}, err
	}
